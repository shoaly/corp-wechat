 ...
 */
namespace CorpWechat\Server;

use EasyWeChat\Core\Exceptions\FaultException;
use EasyWeChat\Core\Exceptions\InvalidArgumentException;
use EasyWeChat\Core\Exceptions\RuntimeException;
use CorpWechat\Encryption\Encryptor;
...
class Guard
{
    ...
    public function serve()
    {
        ...
        if ($str = $this->request->get('echostr')) {
            ...
            $decrypted_echostr = $this->validate($this->token);
            return new Response($decrypted_echostr);
    ...
    public function validate($token)
    {
        ...
        $decrypted_echostr = $this->encryptor->decryptPureMsg($msg_signature, $nonce, $timestamp,$echostr);

        if (!$this->debug && !$decrypted_echostr) {
    ...
    protected function buildReply($to, $from, $message)
    {
        ...
        $transformer = new \EasyWeChat\Server\Transformer();
