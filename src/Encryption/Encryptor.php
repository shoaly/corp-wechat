 ...
 */
namespace CorpWechat\Encryption;
...
class Encryptor
{
    ...
    //企业号的URL 验证握手 都需要解密, 所以这个消息单独拿出来
    public function decryptPureMsg($msgSignature, $nonce, $timestamp, $encrypted)
    {
        Log::info([$msgSignature, $nonce, $timestamp, $encrypted]); 
        $signature = $this->getSHA1($this->token, $timestamp, $nonce, $encrypted);
